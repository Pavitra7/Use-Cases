/*In this experiment, we have one victim main thread, one victim encryption thread having AES 192, one benchmark thread and one attack thread*/
...
K_THREAD_DEFINE(my_benchmark1_thread1, MY_STACK_SIZE,
                do_basic_math_large,NULL,NULL,NULL,
                MY_PRIORITY, 0, 0);
    // Encrypt the message
