/*In this experiment, we have one victim main thread, one victim encryption thread having AES 128 , one benchmark thread and one complex thread*/
...
K_THREAD_DEFINE(my_benchmark1_thread1, MY_STACK_SIZE,
                ...
                MY_PRIORITY, 0, 0);
K_THREAD_DEFINE(my_comp1_thread1, MY_STACK_SIZE,
                do_comp_1, NULL, NULL, NULL,
                MY_PRIORITY, 0, 0);                
    // Encrypt the message
    k_thread_start(my_victim_thread);
    k_thread_start(my_benchmark1_thread1);
    k_thread_start(my_comp1_thread);
